...
- **职责**：聚合 EASBot 内置（`buildin/`）和工具（`tools/`）类技能，供各类 Agent 加载、组合与复用。
- **许可**：MIT（见 [LICENSE](./LICENSE)）。
- **说明文档**：[README.md](./README.md) / [README.en.md](./README.en.md)。
- **元包名**：`@easbot/agent-skills`（仅声明依赖与 `files`，无运行时入口）。
- **两类 scripts 区分**：
  - **根 `scripts/`** —— 项目维护脚本（changelog / version bump / docs 同步 / structure 校验副本），由仓库维护者本地或 CI 调用。
  - **`skills/<category>/<name>/scripts/`** —— 技能包内部脚本，由加载该技能的 Agent 调用。
  两者都遵循 §11 的代码规范，只是归属层级不同。
...
├── .github/                # CI / Issue 模板
└── skills/
    ├── buildin/         # 内置核心技能（不要新增非 builtin 类别）
    │   ├── eas-agent-creation/
    │   ├── eas-agent-evolution/
    │   ├── eas-planning-writer/
    │   ├── eas-prompt-creator/
...
- 新增技能前，确认其类别属于 `buildin` 还是 `tools`，不要自创分类。
...
2. **正文必须有"何时使用 / When to Use"**：明确触发场景与反场景。
3. **正文必须有"快速参考 / Quick Reference"**：列出能力要点而非全量内容。
4. **详细说明拆到 `references/`**，避免 `SKILL.md` 过长。
5. **引用路径**用相对路径（`./references/xxx.md`、`./scripts/yyy.ts`）。
6. 不要把"如何加载本技能"放在首屏（只在文末说明一次即可）。

参考样例：`skills/buildin/eas-skill-using/SKILL.md`。
...
> 校验技能结构请用 `skills/buildin/eas-skill-creator/scripts/quick-validate.ts`（见 §5.2），不要用根 `scripts/` 下任何同名/同义脚本。

### 5.2 技能包内部脚本

| 用途 | 命令 | 依赖 |
| --- | --- | --- |
| 校验单个技能结构 | `npx tsx skills/buildin/eas-skill-creator/scripts/quick-validate.ts <skill-dir>` | `js-yaml` |
| 初始化空技能骨架 | `npx tsx skills/buildin/eas-skill-creator/scripts/init-skill.ts <name> --path <dest> [--resources scripts,references,assets] [--examples]` | 零依赖 |
| 打包为 `.skill`（ZIP） | `npx tsx skills/buildin/eas-skill-creator/scripts/package-skill.ts <skill-dir> [output-dir]` | `js-yaml` + `jszip` |
| 初始化 Agent 配置 | `npx tsx skills/buildin/eas-agent-evolution/scripts/init-agent.ts --workspace <path> [--output <dir>] [--non-interactive] --agent-name <name> --user-name <name> ...` | 零依赖 |
| 备份 Agent 配置 | `npx tsx skills/buildin/eas-agent-evolution/scripts/backup-config.ts [--config-path <path>]` | 零依赖 |
| 校验 Agent 配置 | `npx tsx skills/buildin/eas-agent-evolution/scripts/validate-config.ts [--config-path <path>]` | 零依赖 |
| 增量更新 Agent 配置 | `npx tsx skills/buildin/eas-agent-evolution/scripts/update-agent.ts ...` | 零依赖 |
| 注册定时备份任务 | `npx tsx skills/buildin/eas-agent-evolution/scripts/register-backup-task.ts <register\|list\|delete\|help> [--cron <expr>] [--config-path <path>]` | 由 `@easbot/agent` 提供（见下文说明） |
| 初始化规划三件套 | `npx tsx skills/buildin/eas-planning-writer/scripts/init-planning-session.ts [--output <dir>]` | 零依赖 |
| 检查规划完成度 | `npx tsx skills/buildin/eas-planning-writer/scripts/check-complete.ts` | 零依赖 |

> **`register-backup-task.ts` 的特殊性**：本脚本依赖 `@easbot/agent` 的 `Scheduler` API。它是仓库内**唯一**允许 import `@easbot/agent` 的 scripts 文件；其他脚本仍须遵循 §11 的"零外部依赖"原则。该脚本通过 `package.json` 的 `dependencies."@easbot/agent"` 引用，调用前先 `npm install` 即可。

全仓"事实上的 test suite"（循环校验全部 8 个技能）：

```bash
for s in skills/buildin/*/ skills/tools/*/; do
  [ -f "$s/SKILL.md" ] && npx tsx skills/buildin/eas-skill-creator/scripts/quick-validate.ts "$s"
done
```
...
- **初始化协议模板**：`skills/buildin/eas-agent-evolution/assets/BOOTSTRAP.md` 是 Agent 自初始化的协议来源，**勿**手动编辑该文件。
