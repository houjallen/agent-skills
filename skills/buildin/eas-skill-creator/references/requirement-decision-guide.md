...
- ❌ 引用本技能目录**之外**的绝对路径
- ❌ 引用 `../../` 跨越本技能目录外的相对路径
- ❌ 引用宿主项目内的具体路径（如 `<cwd>/docs/decisions/...`）作为**本技能内部可访问的资源**
- ❌ 假设宿主项目存在特定目录结构（如 `.easbot/skills/`、`docs/decisions/`）

**[MAY] 允许的引用形式：**

- ✅ 本技能目录内部相对路径（如 `[guide.md](requirement-decision-guide.md)`）
- ✅ 通用占位符 `<cwd>/...` 表示"宿主项目根目录下"（仅用于说明产物落地位置）
- ✅ 通用占位符 `<eas-skill-creator>/...` 表示"本技能目录内部"（用于引用本技能模板）

**理由**：本技能可能被不同项目、不同 Agent 框架调用，宿主项目的目录结构可能完全不同。本技能 MUST 在任意宿主项目下都能独立使用，不依赖宿主项目的特定目录布局。
