...
```bash
cp -r <skillPath>/assets/xlsx_template/ /tmp/xlsx_work/
```
...
```bash
python3 <skillPath>/scripts/shared_strings_builder.py \
...
```bash
python3 <skillPath>/scripts/shared_strings_builder.py --file strings.txt \
  > /tmp/xlsx_work/xl/sharedStrings.xml
```
...
```bash
python3 <skillPath>/scripts/xlsx_pack.py /tmp/xlsx_work/ /path/to/output.xlsx
```
