...
```bash
python3 <skillPath>/scripts/formula_check.py /path/to/file.xlsx
```

**JSON output (for programmatic processing):**

```bash
python3 <skillPath>/scripts/formula_check.py /path/to/file.xlsx --json
```

**Single-sheet mode (faster for targeted checks):**

```bash
python3 <skillPath>/scripts/formula_check.py /path/to/file.xlsx --sheet Summary
```

**Summary mode (counts only, no per-cell detail):**

```bash
python3 <skillPath>/scripts/formula_check.py /path/to/file.xlsx --summary
```
...
```bash
python3 <skillPath>/scripts/xlsx_unpack.py /path/to/file.xlsx /tmp/xlsx_inspect/
```
...
python3 <skillPath>/scripts/libreoffice_recalc.py --check

# Run recalculation (default timeout: 60s)
python3 <skillPath>/scripts/libreoffice_recalc.py /path/to/input.xlsx /tmp/recalculated.xlsx

# For large or complex files, extend the timeout
python3 <skillPath>/scripts/libreoffice_recalc.py /path/to/input.xlsx /tmp/recalculated.xlsx --timeout 120
```
...
```bash
python3 <skillPath>/scripts/formula_check.py /tmp/recalculated.xlsx
```
...
python3 <skillPath>/scripts/formula_check.py /path/to/output.xlsx

# Step 2: Dynamic check (if LibreOffice available)
python3 <skillPath>/scripts/libreoffice_recalc.py /path/to/output.xlsx /tmp/recalculated.xlsx
python3 <skillPath>/scripts/formula_check.py /tmp/recalculated.xlsx
```
...
python3 <skillPath>/scripts/formula_check.py /path/to/edited.xlsx --json \
  | python3 -c "
import json, sys
...
python3 <skillPath>/scripts/formula_check.py /path/to/user_file.xlsx --json > /tmp/validation_results.json

# Step 2: Unpack for manual inspection
python3 <skillPath>/scripts/xlsx_unpack.py /path/to/user_file.xlsx /tmp/xlsx_inspect/

# Step 3: Dynamic recalculation
python3 <skillPath>/scripts/libreoffice_recalc.py /path/to/user_file.xlsx /tmp/user_file_recalc.xlsx

# Step 4: Re-validate recalculated file
python3 <skillPath>/scripts/formula_check.py /tmp/user_file_recalc.xlsx --json > /tmp/validation_after_recalc.json
