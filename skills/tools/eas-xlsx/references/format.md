...
```bash
python3 <skillPath>/scripts/xlsx_unpack.py input.xlsx /tmp/xlsx_fmt/
```
...
python3 <skillPath>/scripts/style_audit.py /tmp/xlsx_fmt/unpacked/

# Formula error static scan (must specify a single .xlsx file, does not accept directories)
# Pack first, then scan:
python3 <skillPath>/scripts/xlsx_pack.py /tmp/xlsx_fmt/unpacked/ /tmp/output.xlsx
python3 <skillPath>/scripts/formula_check.py /tmp/output.xlsx
```
...
```bash
python3 <skillPath>/scripts/xlsx_pack.py /tmp/xlsx_fmt/unpacked/ output.xlsx
```
