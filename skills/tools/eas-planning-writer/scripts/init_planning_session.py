#!/usr/bin/env python3
"""
eas-planning-writer 初始化脚本
用于创建和初始化规划文件 (task_plan.md, findings.md, progress.md)
"""
...
def generate_task_plan_content(project_name: str) -> str:
    """
    生成 task_plan.md 模板内容
    """
    return f"""# 任务计划：{project_name}
...
def generate_findings_content() -> str:
    """
    生成 findings.md 模板内容
    """
...
def generate_progress_content() -> str:
    """
    生成 progress.md 模板内容
    """
    return f"""# 进度日志 (Progress Log)
<!-- 
...
def create_planning_files(output_dir: str = './docs/task') -> None:
    """创建规划文件"""
    # 确保输出目录存在
    ...
    if not file_exists(task_plan_path):
        with open(task_plan_path, 'w', encoding='utf-8') as f:
            f.write(generate_task_plan_content('New Project'))
        print('Created task_plan.md')
    else:
        print('task_plan.md already exists, skipping')

    # 创建 findings.md（如果不存在）
    if not file_exists(findings_path):
        with open(findings_path, 'w', encoding='utf-8') as f:
            f.write(generate_findings_content())
        print('Created findings.md')
    else:
        print('findings.md already exists, skipping')

    # 创建 progress.md（如果不存在）
    if not file_exists(progress_path):
        with open(progress_path, 'w', encoding='utf-8') as f:
            f.write(generate_progress_content())
        print('Created progress.md')
    else:
        print('progress.md already exists, skipping')

    print('\nPlanning files initialized successfully!')
    print(f'Files created in: {output_dir}')
    print('Files: task_plan.md, findings.md, progress.md')
    print('\nNext steps:')
    print('   1. Update task_plan.md with your specific goals and phases')
    print('   2. Use findings.md to record your discoveries and decisions')
    print('   3. Update progress.md regularly as you work')
    print('   4. Check your progress regularly using the 5-question reboot test')
...
def main():
    ...
    while i < len(args):
        if args[i] in ('--output', '-o'):
            ...
            print('用法: python init_planning_session.py [--output|-o <directory>]')
            print('初始化规划文件 (task_plan.md, findings.md, progress.md)')
            print('')
            print('选项:')
            print('  --output, -o <directory>  规划文件的输出目录 (默认: ./docs/task)')
            print('  --help, -h                显示此帮助信息')
            sys.exit(0)
        i += 1

    print(f"Initializing planning session in: {output_dir}")
    create_planning_files(output_dir)
