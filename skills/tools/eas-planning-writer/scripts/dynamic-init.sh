#!/bin/bash
# eas-planning-writer 动态初始化脚本 - 从模板文件读取内容
# 用法: ./dynamic-init.sh [project-name]
...
read_template() {
    ...
    if [[ -f "$template_path" ]]; then
        ...
        echo "警告: 未找到模板文件 $template_path, 使用默认内容" >&2
        # 输出默认内容
...
echo "在 ${DATE} 初始化规划文件: $PROJECT_NAME"

# 创建 task_plan.md 如果它不存在
if [ ! -f "task_plan.md" ]; then
    echo "创建 task_plan.md"
    local_content=$(read_template "task_plan.md")
    # 替换占位符 DATE_VAR
    echo "$local_content" | sed "s/DATE_VAR/$DATE/g" > task_plan.md
else
    echo "task_plan.md 已存在，跳过"
fi

# 创建 findings.md 如果它不存在
if [ ! -f "findings.md" ]; then
    echo "创建 findings.md"
    local_content=$(read_template "findings.md")
    # 替换占位符 DATE_VAR
    echo "$local_content" | sed "s/DATE_VAR/$DATE/g" > findings.md
else
    echo "findings.md 已存在，跳过"
fi

# 创建 progress.md 如果它不存在
if [ ! -f "progress.md" ]; then
    echo "创建 progress.md"
    local_content=$(read_template "progress.md")
    # 替换占位符 DATE_VAR
    echo "$local_content" | sed "s/DATE_VAR/$DATE/g" > progress.md
else
