...
def read_template(template_name: str) -> str:
    """从模板目录读取模板文件内容"""
    template_path = Path(__file__).parent / '..' / 'references' / 'templates' / template_name
    try:
        with open(template_path, 'r', encoding='utf-8') as f:
            return f.read()
    except FileNotFoundError:
        print(f"警告: 未找到模板文件 {template_path}, 使用默认内容")
        # 如果找不到模板，返回默认内容
        return get_default_template(template_name)

def get_default_template(template_name: str) -> str:
    """返回默认模板内容"""
    if template_name == 'task_plan.md':
        return f"""# 任务计划：{DATE}
...
- 记录所有错误 - 它们有助于避免重复
"""
    elif template_name == 'findings.md':
...
def create_planning_files(output_dir: str = './docs/task') -> None:
    ...
    if not file_exists(task_plan_path):
        template_content = read_template('task_plan.md')
        with open(task_plan_path, 'w', encoding='utf-8') as f:
            f.write(template_content)
        print('创建 task_plan.md')
    else:
        print('task_plan.md 已存在，跳过')

    # 创建 findings.md（如果不存在）
    if not file_exists(findings_path):
        template_content = read_template('findings.md')
        with open(findings_path, 'w', encoding='utf-8') as f:
            f.write(template_content)
        print('创建 findings.md')
    else:
        print('findings.md 已存在，跳过')

    # 创建 progress.md（如果不存在）
    if not file_exists(progress_path):
        template_content = read_template('progress.md')
        with open(progress_path, 'w', encoding='utf-8') as f:
            f.write(template_content)
        print('创建 progress.md')
    else:
        print('progress.md 已存在，跳过')

    print('\n规划文件初始化成功！')
    print(f'文件创建于: {output_dir}')
    print('文件: task_plan.md, findings.md, progress.md')
    print('\n后续步骤:')
    print('   1. 更新 task_plan.md 中的特定目标和阶段')
    print('   2. 使用 findings.md 记录您的发现和决策')
    print('   3. 定期更新 progress.md')
    print('   4. 使用 5 个问题重启测试定期检查您的进度')
    ...
    while i < len(args):
        if args[i] in ('--output', '-o'):
            if i + 1 < len(args):
                ...
                print('❌ 错误: --output 需要一个目录路径')
                sys.exit(1)
        elif args[i] in ('--help', '-h'):
            print('用法: python dynamic-init.py [--output|-o <directory>]')
